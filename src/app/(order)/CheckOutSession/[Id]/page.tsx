...
const Page = () => {
  ...
  const Schemacheckout = z.object({
    details: z
      .string()
      .min(2, "City name is required (min 2 characters)")
      .max(100, "City name is required (max 100 characters"),
    phone: z
      .string()
      .min(10, "Enter a valid Egyptian phone number")
      .max(15, "max number is 15"),
    city: z
      .string()
      .min(2, "City name is required (min 2 characters)")
      .max(100),
    ...
    <>
      ...
      <div className="bg-blue-50 m-4 p-4 rounded-b-2xl">
        <div className="relative ">
          ...
          <p className="bg-blue-50 border-blue-100 rounded-2xl border-2 px-8">
            please ensure address is accurate
          </p>
        </div>
        <Form {...CheckOutForm}>
          <form onSubmit={CheckOutForm.handleSubmit(HandleCheckOut)}>
            ...
            <Button
              type="submit"
              className="relative mt-4 overflow-hidden p-6 cursor-pointer  w-full bg-blue-950 text-white group"
            >
