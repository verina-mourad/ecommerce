...
function page() {
  ...
  async function HandleLogin(value:RegisterType){
    setloading(true)
    try{
      const res=await fetch(`${process.env.NEXT_PUPLIC_BASE_URL}/api/v1/auth/forgotPasswords`,{
        method:'POST',
  ...
  <div className='container mx-auto shadow-2xl max-w-[700px]  h-[700px] flex '>
   ...
   <div className='mx-16 gap-3 flex flex-col'>
    ...
    <h2 className='text-gray-500'>Step 1: Enter your registered email to <br /> receive reset instructions.</h2>
  ...
  <Button className='bg-gray-50 shadow-2xl px-4 text-gray-400 rounded-none'>
    <FaKey className='size-6' />
  </Button>

  <div className="h-[2px] w-52 bg-gray-300"></div>

  <Button className='bg-gray-50 shadow-2xl px-4 text-gray-400 rounded-l-none'>
